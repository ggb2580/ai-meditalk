...
public class AppointmentTools {
    ...
    @Tool(name="appointment", value = "根据参数，先执行工具方法uu,,查询是否可预约，并直接给用户回答是否可预约，并让用户确认所有预约信息，用户确认后再进行预约。")
    ...
    @Tool(name = "deleteAppointment",value = "删除用户的预约信息,只需要获取到用户的身份证号和预约日期即可，姓名是可选项")
    public String removeAppointment(@P(value = "用户姓名",required = false) String name,
                                    @P(value = "身份证",required = true) String idCard,
                                    @P(value = "日期",required = true)String date){

        System.out.println("===== 大模型传过来的数据 =====");
        ...
        //删除
        boolean b = appointmentService.removeAppointment(appointment);
    ...
    @Tool(name = "queryAppointment", value="根据科室名称，日期，时间和医生查询是否有号源，并返回给用户")
